...
class ReportUserCashDayData extends IReportUserCashDayData implements IDaySchedule
{
    ...
    protected $reportAdapter = "App\Http\Adapter\Report\ReportUserCashDayAdapter";
    // protected $noPre = "SCR";
    protected $reportData="App\Data\Report\ReportUserCashDayData";
    ...
    public function run()
    {
        ...
        while(strtotime($tmp_end)<=strtotime($end))
        {
            ...
            while($pageIndex<=($result["pageCount"])){  
                foreach($result["items"] as $resultitem){
                    $this->makeDayReport($resultitem->id, $tmp_start, $tmp_end);
                }
                $pageIndex ++;
                $result = $userFac->query($filter, $pageSize, $pageIndex);   
            }
