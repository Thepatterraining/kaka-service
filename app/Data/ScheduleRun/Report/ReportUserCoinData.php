...
class ReportUserCoinData implements IDaySchedule
{
    //
    public function run()
    {
        ...
        $info=$reportUserCoinDayData->getTop(date_format($endDate, "Y-m-d"), $idCount);
        ...
        while(empty($info))
        {
            ...
            $idCount=$userFac->getMaxIdDay($endDate);
            $info=$reportUserCoinDayData->getTop(date_format($date, "Y-m-d"), $idCount);
            date_add($date, date_interval_create_from_date_string("-1 days"));
        ...
        while(strtotime($tmp_end)<=strtotime($end))
        {
            ...
            while($pageIndex<=($result["pageCount"])){  
                foreach($result["items"] as $resultitem){
                    $res=$reportUserCoinDayData->makeDayReport($resultitem->id, $tmp_start, $tmp_end);
                }
                $pageIndex ++;
                $result = $userFac->query($filter, $pageSize, $pageIndex);   
            }
