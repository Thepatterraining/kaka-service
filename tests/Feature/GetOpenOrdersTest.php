...
class GetOpenOrdersTest extends TestCase
{
    ...
    public function testAddProductInfo()
    {
        ...
        $coinType = 'KKC-BJ0001';
        $paypwd = '123qweA';
        $response = $this->getOpenOrders($token, $pageIndex, $pageSize, $coinType, $paypwd);
        dump($response);
        $this->assertCount(4, $response);
        $this->assertEquals(0, $response['code']);
    }

    protected function getOpenOrders($token, $pageIndex, $pageSize, $coinType, $paypwd)
    {
        $response = $this->json(
            'POST', '/api/login/trade/getopenorders', array(
            "accessToken"=>$token,
            "pageIndex"=>$pageIndex,
            "pageSize"=>$pageSize,
            "coinType"=>$coinType,
