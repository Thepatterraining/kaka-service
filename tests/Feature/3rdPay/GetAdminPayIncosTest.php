...
class GetAdminPayIncosTest extends TestCase
{
    /**
    ...
    public function testGetChannels()
    {
        ...
        $pageIndex = 1;
        $pageSize = 10;
        $response = $this->getChannels($token, $pageIndex, $pageSize);
        dump($response);
        // $this->assertCount(4,$response);
        //$this->assertEquals(0,$response['code']);
    }

    protected function getChannels($token, $pageIndex, $pageSize)
    {
        $response = $this->json(
            'POST', '/api/admin/get/payincomeds', [
            "accessToken"=>$token,
            "pageIndex"=>$pageIndex,
            "pageSize"=>$pageSize,
            ]
