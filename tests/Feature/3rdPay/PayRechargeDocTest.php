...
class PayRechargeDocTest extends TestCase
{
    /**
    ...
    public function testGetChannels()
    {
        ...
        //开始登陆
        dump('手机号登陆');
        $beforeTime = mktime(0, 0, 0, 6, 6, 2017);
        $startTime = date('Y-m-d H:i:s', $beforeTime);
        //var_dump($startTime);
        $phone = '13263463442';
        ...
        //发起充值：
        dump('确认入账：');
        $userRechargeData = new \App\Data\Cash\UserRechargeData();
        //$rechargeNo = 'CR2017050823031355591';
        $channelid = '2';
        //$response = $userRechargeData->ThirdPartyRechargeTrue($rechargeNo,$channelid);
        $nextTime = mktime(0, 0, 0, 6, 7, 2017);
        $endTime = date('Y-m-d H:i:s', $nextTime);
        //var_dump($endTime);
        // $response = $userRechargeData->ThirdPartyRechargeIncomedocs($startTime,$endTime,$channelid);
        // $this->assertCount(4,$response);
        // $this->assertEquals(0,$response['code']);
