...
class ActivityGetVoucherTest extends TestCase
{
    /**
    ...
    public function testAddProductInfo()
    {
        ...
        dump('领取活动奖励');
        $activityNo = 1400;
        $response = $this->activitygetvoucher($token, $activityNo);
        dump($response);
        $this->assertCount(4, $response);
        $this->assertEquals(0, $response['code']);
    }

    protected function activitygetvoucher($token, $activityNo)
    {
        $response = $this->json(
            'POST', '/api/login/user/activitygetvoucher', array(
            "accessToken"=>$token,
