...
class AddUserVoucherTest extends TestCase
{
    /**
     ...
     * @covers
     */
    public function testAddUserVoucher()
    {
        dump('测试开始');
        dump("Require Token:");
        ...
        dump('用户兑换邀请码：');
        $code = '123456789';
        $response = $this->addCode($token, $code);
        dump($response);
    ...
    protected function addCode($token, $code)
    {
        $response = $this->json(
            'POST', '/api/user/adduservoucher', [
            "accessToken"=>$token,
            'activityCode'=>$code,
            ]
        );
