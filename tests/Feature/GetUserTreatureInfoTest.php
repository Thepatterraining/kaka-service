...
class GetUserTreatureInfoTest extends TestCase
{
    /*
     ...
     * @covers
     */
    public function testGetUserTreatureInfo()
    {
        dump('测试开始');
        dump("Require Token:");
        ...
        //查询资产概况
        dump('查询资产概况：');
        // $response = $this->getTreature($token);
        // dump($response);
        // $this->assertCount(4, $response);
        // $this->assertEquals(0, $response['code']);
    ...
    protected function getTreature($token)
    {
        $response = $this->json(
            'POST', '/api/user/get/getusertreature', [
            "accessToken"=>$token
            ]
        );
        $response->assertStatus(200);
