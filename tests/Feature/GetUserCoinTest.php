...
class GetUserCoinTest extends TestCase
{
    /**
    ...
    public function testGet()
    {
        ...
        dump('查看用户持有的项目详情：');
        $coinType = 'KKC-BJ0001';
        $response = $this->getUserCoin($token, $coinType);
        dump($response);
        $this->assertCount(4, $response);
        $this->assertEquals(0, $response['code']);
    }

    protected function getUserCoin($token, $coinType)
    {
        $response = $this->json(
            'POST', '/api/login/user/getusercoin', [
            "accessToken"=>$token,
            'coinType'=>$coinType,
            ]
        );
