...
class GetUserOrderListTest extends TestCase
{
    /**
     ...
     * @covers
     */
    public function testGetUserOrderList()
    {
        dump('测试开始');
        dump("Require Token:");
        ...
        $pageIndex = 1;
        $pageSize = 10;
        $response = $this->getorderlist($token, $pageIndex, $pageSize);
        dump($response);
        $this->assertCount(4, $response);
        $this->assertEquals(0, $response['code']);
    ...
    protected function getorderlist($token, $pageIndex, $pageSize)
    {
        $response = $this->json(
            'POST', '/api/trade/getorderlist', [
            "accessToken"=>$token,
            'pageIndex'=>$pageIndex,
            'pageSize'=>$pageSize,
            ]
