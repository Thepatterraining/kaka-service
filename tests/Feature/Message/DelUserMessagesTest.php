...
class DelUserMessagesTest extends TestCase
{
    /**
     ...
     * @covers
     */
    public function testDelUserMessages()
    {
        dump('测试开始');
        dump("Require Token:");
        ...
        //删除消息列表
        dump('删除消息列表：');
        $response = $this->delMessages($token);
        dump($response);
        $this->assertCount(4, $response);
        $this->assertEquals(0, $response['code']);
    }

    protected function delMessages($token)
    {
        $response = $this->json(
            'POST', '/api/user/delusermsg', [
            "accessToken"=>$token,
            ]
        );
        $response->assertStatus(200);
