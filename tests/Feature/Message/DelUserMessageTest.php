...
class DelUserMessageTest extends TestCase
{
    /**
     ...
     * @covers
     */
    public function testDelUserMessage()
    {
        dump('测试开始');
        dump("Require Token:");
        ...
        //删除消息
        dump('删除消息：');
        $no = 'MSG2017051122243517486';
        $response = $this->delMessage($token, $no);
        dump($response);
        $this->assertCount(4, $response);
        $this->assertEquals(0, $response['code']);
    }

    protected function delMessage($token, $no)
    {
        $response = $this->json(
