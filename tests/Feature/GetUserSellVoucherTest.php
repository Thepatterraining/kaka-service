...
class GetUserSellVoucherTest extends TestCase
{
    /**
    ...
    public function testGetUserSellVoucher()
    {
        ...
        $sell = $sellData->get(1);
        // $sellNo = 'TS2017061015155157735';
        $sellNo = $sell->sell_no;
        $price = 3000;
        $response = $this->getList($token, $sellNo, $price);
        dump($response);
        $this->assertCount(4, $response);
        $this->assertEquals(0, $response['code']);
    }
    ...
    protected function getList($token, $sellNo, $price)
    {
        $response = $this->json(
            'POST', '/api/user/getvoucherinfo', array(
            "accessToken"=>$token,
            "sellNo"=>$sellNo,
            "price"=>$price,
            )
