...
class GetUserCoinsCountTest extends TestCase
{
    /**
     ...
     * @covers
     */
    public function testGetUserCoinsCount()
    {
        dump('测试开始');
        dump("Require Token:");
    ...
    protected function getList($token)
    {
        $response = $this->json(
            'POST', '/api/user/getcoinscount', array(
            "accessToken"=>$token,
            )
        );
        $response->assertStatus(200);
