...
class GetUserSellsTest extends TestCase
{
    /**
    ...
    public function testGet()
    {
        ...
        $pageIndex = 1;
        $pageSize = 10;
        $response = $this->getUserSells($token, $pageIndex, $pageSize);
        dump($response);
        $this->assertCount(4, $response);
        $this->assertEquals(0, $response['code']);
    ...
    protected function getUserSells($token, $pageIndex, $pageSize)
    {
        $response = $this->json(
            'POST', '/api/user/gettranactionsell', [
            "accessToken"=>$token,
            'pageIndex'=>$pageIndex,
            'pageSize'=>$pageSize,
            ]
