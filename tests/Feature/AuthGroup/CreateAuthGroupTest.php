...
class CreateAuthGroupTest extends TestCase
{
    /**
    ...
    public function testCreatePay()
    {
        ...
        $data['groupName'] = 'phpunit';
        $data['groupNote'] = '测试';
        $response = $this->create($token, $data);

        
    }

    protected function create($token, $data)
    {
        $response = $this->json(
            'POST', '/api/admin/auth/createauthgroup', array(
            "accessToken"=>$token,
            "data"=>$data,
            )
        );
