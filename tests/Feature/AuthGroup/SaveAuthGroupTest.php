...
class SaveAuthGroupTest extends TestCase
{
    /**
    ...
    public function testCreatePay()
    {
        ...
        $groupid = 1;
        $data['groupName'] = 'papapa2';
        $data['groupNote'] = '啪啪啪改版';
        $response = $this->create($token, $groupid, $data);
        dump($response);
        $this->assertCount(4, $response);
        $this->assertEquals(0, $response['code']);
    ...
    protected function create($token, $groupid, $data)
    {
        $response = $this->json(
            'POST', '/api/admin/auth/saveauthgroup', array(
            "accessToken"=>$token,
            "groupid"=>$groupid,
            "data"=>$data
            )
