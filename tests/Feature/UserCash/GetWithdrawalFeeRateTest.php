...
class GetWithdrawalFeeRateTest extends TestCase
{
    /**
    ...
    public function testGet()
    {
        ...
        //查看提现费率
        dump('查看提现费率：');
        $response = $this->getWithFeeRate($token);
        dump($response);
        $this->assertCount(4, $response);
        $this->assertEquals(0, $response['code']);
    }

    protected function getWithFeeRate($token)
    {
        $response = $this->json(
            'POST', '/api/user/get/cashwithdrawalfeerate', [
            "accessToken"=>$token,
            ]
        );
        $response->assertStatus(200);
