...
class AddActivityGroupTest extends TestCase
{
    /**
     ...
     * @covers
     */
    public function testAddActivityGroup()
    {
        dump('测试开始');
        dump("Require Token:");
        ...
        $name = '测试分组';
        $type = 'AGT01';
        $response = $this->addInfo($token, $name, $type);
        dump($response);
        $this->assertCount(4, $response);
        $this->assertEquals(0, $response['code']);
    }

    protected function addInfo($token, $name, $type)
    {
        $response = $this->json(
            'POST', '/api/admin/activity/addgroup', array(
            "accessToken"=>$token,
            "name"=>$name,
            "type"=>$type,
            )
