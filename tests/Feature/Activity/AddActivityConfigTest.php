...
class AddActivityConfigTest extends TestCase
{
    /**
    ...
    public function testAddActivityConfig()
    {
        ...
        dump('添加活动配置：');
        $data = [];
        $response = $this->addConfig($token, $data);
        ...
        dump('添加活动配置：');
        $data['usertype'] = 'UT00';
        $data['code'] = 'SA2017050723070470137';
        $response = $this->addConfig($token, $data);
        dump($response);
        if (count($response) == 3) {
            $this->assertCount(3, $response);
            $this->assertEquals(809003, $response['code']);
        } else {
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
        }
    }
    protected function addConfig($token, $data)
    {
        $response = $this->json(
            'POST', '/api/admin/addactivityconfig', [
            "accessToken"=>$token,
            'data'=>$data,
            ]
        );
