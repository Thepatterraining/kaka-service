...
class GetTradesTest extends TestCase
{
    /**
    ...
    public function testAddProductInfo()
    {
        ...
        $coinType = 'KKC-BJ0001';
        $count = 10;
        // $response = $this->gettrades($token,$coinType, $count);
        // dump($response);
        // $this->assertCount(4,$response);
        // $this->assertEquals(0,$response['code']);
    }

    protected function gettrades($token, $coinType, $count)
    {
        $response = $this->json(
            'POST', '/api/token/trade/gettransactions', array(
            // "accessToken"=>$token,
            "coinType"=>$coinType,
            // "count"=>$count,
            )
