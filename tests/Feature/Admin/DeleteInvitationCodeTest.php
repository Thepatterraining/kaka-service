...
class DeleteInvitationCodeTest extends TestCase
{
    /**
     ...
     * @covers
     */
    public function testDeleteInvitationCode()
    {
        dump('测试开始');
        dump("Require Token:");
        ...
        dump('删除用户邀请码：');
        $code = 'jS4JUAiR';
        $response = $this->delCode($token, $code);
        dump($response);
        if (count($response) == 3) {
    ...
    protected function delCode($token, $code)
    {
        $response = $this->json(
            'POST', '/api/admin/delinvitationcode', [
            "accessToken"=>$token,
            'code'=>$code,
            ]
        );
