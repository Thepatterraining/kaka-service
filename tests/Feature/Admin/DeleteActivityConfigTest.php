...
class DeleteActivityConfigTest extends TestCase
{
    /**
     ...
     * @covers
     */
    public function testDeleteActivityConfig()
    {
        dump('测试开始');
        dump("Require Token:");
        ...
        dump('删除活动配置：');
        $userType = 'UT02';
        $response = $this->delConfig($token, $userType);
        dump($response);
        if (count($response) == 3) {
        ...
        $response = $this->delConfig($token, $userType);
        $this->assertCount(3, $response);
        $this->assertEquals(802002, $response['code']);
    }
    ...
    protected function delConfig($token, $userType)
    {
        $response = $this->json(
            'POST', '/api/admin/delactivityconfig', [
            "accessToken"=>$token,
            'userType'=>$userType,
            ]
        );
