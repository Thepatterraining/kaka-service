...
class DeleteSubwaystationTest extends TestCase
{
    /**
    ...
    public function testRegion()
    {
        ...
        dump('删除地铁站');
            $id = 1;
            $response = $this->deletesubwaystation($token, $id);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
    ...
    protected function deletesubwaystation($token, $id)
    {
        $response = $this->json(
            'POST', '/api/v2/admin/region/deletesubwaystation', array(
            "accessToken"=>$token,
            "id"=>$id,
            )
        );
