...
class GetCountrisTest extends TestCase
{
    /**
    ...
    public function testRegion()
    {
        ...
        dump('查询国家');
            $pageIndex = 1;
            $pageSize = 10;
            $response = $this->getcountris($token, $pageIndex, $pageSize);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
    ...
    protected function getcountris($token, $pageIndex, $pageSize)
    {
        $response = $this->json(
            'POST', '/api/v2/admin/region/getcountris', array(
            "accessToken"=>$token,
            "pageIndex"=>$pageIndex,
            "pageSize"=>$pageSize,
            )
