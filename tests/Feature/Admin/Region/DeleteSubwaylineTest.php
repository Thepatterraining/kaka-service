...
class DeleteSubwaylineTest extends TestCase
{
    /**
    ...
    public function testRegion()
    {
        ...
        dump('删除地铁线路');
            $id = 1;
            $response = $this->deletesubwayline($token, $id);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
    ...
    protected function deletesubwayline($token, $id)
    {
        $response = $this->json(
            'POST', '/api/v2/admin/region/deletesubwayline', array(
            "accessToken"=>$token,
            "id"=>$id,
            )
        );
