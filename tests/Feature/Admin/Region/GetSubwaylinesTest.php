...
class GetSubwaylinesTest extends TestCase
{
    /**
    ...
    public function testRegion()
    {
        ...
        dump('查询地铁线路');
            $pageIndex = 1;
            $pageSize = 10;
            $response = $this->getsubwaylines($token, $pageIndex, $pageSize);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
    ...
    protected function getsubwaylines($token, $pageIndex, $pageSize)
    {
        $response = $this->json(
            'POST', '/api/v2/admin/region/getsubwaylines', array(
            "accessToken"=>$token,
            "pageIndex"=>$pageIndex,
            "pageSize"=>$pageSize,
            )
