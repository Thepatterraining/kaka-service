...
class GetDistrictsTest extends TestCase
{
    ...
    public function testRegion()
    {
        ...
        dump('查询区');
            $pageIndex = 1;
            $pageSize = 10;
            $response = $this->getdistricts($token, $pageIndex, $pageSize);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
    ...
    protected function getdistricts($token, $pageIndex, $pageSize)
    {
        $response = $this->json(
            'POST', '/api/v2/admin/region/getdistricts', array(
            "accessToken"=>$token,
            "pageIndex"=>$pageIndex,
            "pageSize"=>$pageSize,
            )
