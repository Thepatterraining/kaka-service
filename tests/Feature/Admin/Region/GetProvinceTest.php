...
class GetProvinceTest extends TestCase
{
    /**
    ...
    public function testRegion()
    {
        ...
        dump('查询省');
            $id = 1;
            $response = $this->getprovince($token, $id);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
    ...
    protected function getprovince($token, $id)
    {
        $response = $this->json(
            'POST', '/api/v2/admin/region/getprovince', array(
            "accessToken"=>$token,
            "id"=>$id,
            )
        );
