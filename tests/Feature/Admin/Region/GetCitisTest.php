...
class GetCitisTest extends TestCase
{
    /**
    ...
    public function testRegion()
    {
        ...
        dump('查询城市');
            $pageIndex = 1;
            $pageSize = 10;
            $response = $this->getcitis($token, $pageIndex, $pageSize);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
    ...
    protected function getcitis($token, $pageIndex, $pageSize)
    {
        $response = $this->json(
            'POST', '/api/v2/admin/region/getcitis', array(
            "accessToken"=>$token,
            "pageIndex"=>$pageIndex,
            "pageSize"=>$pageSize,
            )
