...
class SaveDistrictTest extends TestCase
{
    ...
    public function testRegion()
    {
        ...
        dump('添加区');
            ...
            $data['cityName'] = '北京';
            $data['data'] = $data;
            $districtData = new \App\Data\Region\DistrictData;
            $model = $districtData->newitem();
            $id = $model->pluck('id')->first();
            dump($id);
            // $id = 1;
        if ($id > 0) {
            $response = $this->savedistrict($token, $data, $id);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
        }
    ...
    protected function savedistrict($token, $data, $id)
    {
        $response = $this->json(
            'POST', '/api/v2/admin/region/savedistrict', array(
            "accessToken"=>$token,
            "data"=>$data,
            "id"=>$id,
            )
