...
class GetSubwaystationTest extends TestCase
{
    /**
    ...
    public function testRegion()
    {
        ...
        dump('查询地铁站');
            $id = 1;
            $response = $this->getsubwaystation($token, $id);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
    ...
    protected function getsubwaystation($token, $id)
    {
        $response = $this->json(
            'POST', '/api/v2/admin/region/getsubwaystation', array(
            "accessToken"=>$token,
            "id"=>$id,
            )
        );
