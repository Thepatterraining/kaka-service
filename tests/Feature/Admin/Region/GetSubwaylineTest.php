...
class GetSubwaylineTest extends TestCase
{
    /**
    ...
    public function testRegion()
    {
        ...
        dump('查询地铁线路');
            $id = 1;
            $response = $this->getsubwayline($token, $id);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
    ...
    protected function getsubwayline($token, $id)
    {
        $response = $this->json(
            'POST', '/api/v2/admin/region/getsubwayline', array(
            "accessToken"=>$token,
            "id"=>$id,
            )
        );
