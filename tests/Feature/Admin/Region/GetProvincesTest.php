...
class GetProvincesTest extends TestCase
{
    ...
    public function testRegion()
    {
        ...
        dump('查询省');
            $pageIndex = 1;
            $pageSize = 10;
            $response = $this->getprovinces($token, $pageIndex, $pageSize);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
    ...
    protected function getprovinces($token, $pageIndex, $pageSize)
    {
        $response = $this->json(
            'POST', '/api/v2/admin/region/getprovinces', array(
            "accessToken"=>$token,
            "pageIndex"=>$pageIndex,
            "pageSize"=>$pageSize,
            )
