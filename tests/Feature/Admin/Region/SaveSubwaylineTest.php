...
class SaveSubwaylineTest extends TestCase
{
    /**
    ...
    public function testRegion()
    {
        ...
        dump('修改地铁线路');
            ...
            $data['cityName'] = '北京';
            $data['data'] = $data;
            $subwaylineData = new \App\Data\Region\SubwaylineData;
            $model = $subwaylineData->newitem();
            $id = $model->pluck('id')->first();
            dump($id);
            // $id = 1;
        if ($id > 0) {
            $response = $this->savesubwayline($token, $data, $id);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
        }
    ...
    protected function savesubwayline($token, $data, $id)
    {
        $response = $this->json(
            'POST', '/api/v2/admin/region/savesubwayline', array(
            "accessToken"=>$token,
            "data"=>$data,
            "id"=>$id,
            )
