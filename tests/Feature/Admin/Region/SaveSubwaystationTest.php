...
class SaveSubwaystationTest extends TestCase
{
    /**
    ...
    public function testRegion()
    {
        ...
        dump('修改地铁站');
            ...
            $data['cityName'] = '北京';
            $data['districtid'] = '1';
            $data['districtName'] = '丰台';
            $data['data'] = $data;
            $subwaystationData = new \App\Data\Region\SubwaystationData;
            $model = $subwaystationData->newitem();
            $id = $model->pluck('id')->first();
            dump($id);
            // $id = 1;
        if ($id > 0) {
            $response = $this->savesubwaystation($token, $data, $id);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
        }
    ...
    protected function savesubwaystation($token, $data, $id)
    {
        $response = $this->json(
            'POST', '/api/v2/admin/region/savesubwaystation', array(
            "accessToken"=>$token,
            "data"=>$data,
            "id"=>$id,
            )
