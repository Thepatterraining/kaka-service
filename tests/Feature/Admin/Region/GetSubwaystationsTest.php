...
class GetSubwaystationsTest extends TestCase
{
    /**
    ...
    public function testRegion()
    {
        ...
        dump('查询地铁站');
            $pageIndex = 1;
            $pageSize = 10;
            $response = $this->getsubwaystations($token, $pageIndex, $pageSize);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
    ...
    protected function getsubwaystations($token, $pageIndex, $pageSize)
    {
        $response = $this->json(
            'POST', '/api/v2/admin/region/getsubwaystations', array(
            "accessToken"=>$token,
            "pageIndex"=>$pageIndex,
            "pageSize"=>$pageSize,
            )
