...
class SaveCountryTest extends TestCase
{
    /**
    ...
    public function testRegion()
    {
        ...
        dump('修改国家');
            ...
            $data['code'] = 'CN';
            $data['telcode'] = '86';
            $data['fullName'] = '中华人民共和国';
            $data['data'] = $data;
            $countryData = new \App\Data\Region\CountryData;
            $model = $countryData->newitem();
            $id = $model->pluck('id')->first();
            dump($id);
            // $id = 1;
        if ($id > 0) {
            $response = $this->savecountry($token, $data, $id);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
        }
    ...
    protected function savecountry($token, $data, $id)
    {
        $response = $this->json(
            'POST', '/api/v2/admin/region/savecountry', array(
            "accessToken"=>$token,
            "data"=>$data,
            "id"=>$id,
            )
