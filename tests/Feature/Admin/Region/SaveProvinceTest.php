...
class SaveProvinceTest extends TestCase
{
    /**
    ...
    public function testRegion()
    {
        ...
        dump('修改省');
            ...
            $data['country'] = 'CN';
            $data['data'] = $data;
            $provinceData = new \App\Data\Region\ProvinceData;
            $model = $provinceData->newitem();
            $id = $model->pluck('id')->first();
            dump($id);
            // $id = 1;
        if ($id > 0) {
            $response = $this->saveprovince($token, $data, $id);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
        }
    ...
    protected function saveprovince($token, $data, $id)
    {
        $response = $this->json(
            'POST', '/api/v2/admin/region/saveprovince', array(
            "accessToken"=>$token,
            "data"=>$data,
            "id"=>$id,
            )
