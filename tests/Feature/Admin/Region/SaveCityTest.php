...
class SaveCityTest extends TestCase
{
    /**
    ...
    public function testRegion()
    {
        ...
        dump('修改城市');
            ...
            $data['country'] = 1;
            $data['data'] = $data;
            $cityData = new \App\Data\Region\CityData;
            $model = $cityData->newitem();
            $id = $model->pluck('id')->first();
            dump($id);
            // $id = 1;
        if ($id > 0) {
            $response = $this->savecity($token, $data, $id);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
        }
    ...
    protected function savecity($token, $data, $id)
    {
        $response = $this->json(
            'POST', '/api/v2/admin/region/savecity', array(
            "accessToken"=>$token,
            "data"=>$data,
            "id"=>$id,
            )
