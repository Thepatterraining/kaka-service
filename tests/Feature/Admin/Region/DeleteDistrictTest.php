...
class DeleteDistrictTest extends TestCase
{
    /**
    ...
    public function testRegion()
    {
        ...
        dump('删除区');
            $id = 1;
            $response = $this->deletedistrict($token, $id);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
    ...
    protected function deletedistrict($token, $id)
    {
        $response = $this->json(
            'POST', '/api/v2/admin/region/deletedistrict', array(
            "accessToken"=>$token,
            "id"=>$id,
            )
        );
