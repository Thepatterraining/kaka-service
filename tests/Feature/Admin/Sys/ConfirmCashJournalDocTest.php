...
class ConfirmCashJournalDocTest extends TestCase
{
    /**
    ...
    public function testRegion()
    {
        ...
        //内部转账审核
        dump('内部转账审核');
            $data = new \App\Data\Sys\CashJournalDocData;
            $model = $data->newitem();
            $where['syscash_journaldoc_success'] = 0;
            $no = $model->where($where)->pluck('syscash_journaldoc_no')->first();
            dump($no);
            // $no = 'SCJD2017080214391459793';
        if (!empty($no)) {
            $confirm = true;
    ...
    {
        $response = $this->json(
            'POST', '/api/admin/cash/confirmcashjournaldoc', array(
            "accessToken"=>$token,
            "no"=>$no,
            "confirm"=>$confirm,
            )
