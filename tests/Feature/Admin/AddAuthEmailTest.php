...
class AddAuthEmailTest extends TestCase
{
    /**
     ...
     * @covers
     */
    public function testAddAuthEmail()
    {
        dump('测试开始');
        dump("Require Token:");
        ...
        dump('开始新建邮箱');
        $email="qwqw@111.com";
        $response = $this->addInfo($token, $email);
        dump($response);
        if (count($response) === 3) {
            $this->assertCount(3, $response);
            $this->assertEquals(801022, $response['code']);
        } else {
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
        }
    ...
    protected function addInfo($token,$email)
    {
        $response = $this->json(
            'POST', '/api/admin/addauthemail', array(
            "accessToken"=>$token,
            "email"=>$email,
            )
        );
