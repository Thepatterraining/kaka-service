...
class DeleteTrendTest extends TestCase
{
    /**
     ...
     * @covers
     */
    public function testDeleteTrend()
    {
        dump('测试开始');
        dump("Require Token:");
        ...
        dump('开始删除');
        $id = 2;
        $response = $this->deleteTrend($token, $id);
        dump($response);
        $this->assertCount(4, $response);
        $this->assertEquals(0, $response['code']);
    }
    ...
    protected function deleteTrend($token, $id)
    {
        $response = $this->json(
            'POST', '/api/product/deletetrend', array(
            "accessToken"=>$token,
            "id"=>$id,
            )
        );
