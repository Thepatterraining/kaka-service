...
class GetActivityConfigInfoTest extends TestCase
{
    /**
    ...
    public function testGetActivityConfigInfo()
    {
        ...
        //查询活动配置列表
        $userType = 'UT01';
        $response = $this->getInfo($token, $userType);
        dump($response);
        if (count($response) == 3) {
    ...
    protected function getInfo($token, $userType)
    {
        $response = $this->json(
            'POST', '/api/admin/getactivityconfiginfo', [
            "accessToken"=>$token,
            "userType"=>$userType,
            ]
        );
