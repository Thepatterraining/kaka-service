...
class SaveTrendTest extends TestCase
{
    /**
    ...
    public function testSaveTrend()
    {
        ...
        dump('开始修改产品');
        $data['price'] = 2000;
        $data['pricetype'] = 'PROP01';
        $data['no'] = 'KKC-BJ0001';
        $data['time'] = '2016-03-01';
        $id = 1;
        $response = $this->addInfo($token, $data, $id);
        dump($response);
        $this->assertCount(4, $response);
        $this->assertEquals(0, $response['code']);
    }

    protected function addInfo($token, $data, $id)
    {
        $response = $this->json(
            'POST', '/api/product/savetrend', array(
            "accessToken"=>$token,
            "data"=>$data,
            "id"=>$id,
            )
