...
class SaveInvitationCodeTest extends TestCase
{
    /**
    ...
    public function testSave()
    {
        ...
        $data['activityno'] = 666;
        $data['userid'] = 1;
        $response = $this->saveCode($token, $code, $data);
        dump($response);
        if (count($response) == 4) {
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
    ...
    protected function saveCode($token, $code, $data)
    {
        $response = $this->json(
            'POST', '/api/admin/saveinvitationcode', [
            "accessToken"=>$token,
            'code'=>$code,
            'data'=>$data
            ]
