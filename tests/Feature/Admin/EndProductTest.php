...
class EndProductTest extends TestCase
{
    ...
    public function testProduct()
    {
        ...
        $where['product_status'] = \App\Data\Product\InfoData::$PRODUCT_STATUS_NOT_START;
        $product = $productData->find($where);

        if (!empty($product)) {
            $productNo = $product->product_no;
            $confirm = rand(0, 1) === 1 ? true : false;
            // $productNo = 'PRO2017061611430798071';
            $response = $this->endProduct($token, $confirm, $productNo);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
        }
    ...
    protected function endProduct($token, $productNo)
    {
        $response = $this->json(
            'POST', '/api/admin/product/end', array(
            "accessToken"=>$token,
            "productNo"=>$productNo,
            )
        );
