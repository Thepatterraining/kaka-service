...
class GetUserInvitationCodeInfoTest extends TestCase
{
    /**
    ...
    public function testGet()
    {
        ...
        dump('查看用户活动邀请码详细：');
        $code = 'PBP13ZGd';
        $response = $this->getUserActivityInfo($token, $code);
        dump($response);
        if (count($response) == 3) {
    ...
    protected function getUserActivityInfo($token, $code)
    {
        $response = $this->json(
            'POST', '/api/admin/getuserinvitationcodeinfo', [
            "accessToken"=>$token,
            'code'=>$code,
            ]
        );
