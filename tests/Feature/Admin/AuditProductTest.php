...
class AuditProductTest extends TestCase
{
    /**
    ...
    public function testProduct()
    {
        ...
        if (!empty($product)) {
            ...
            $response = $this->auditProduct($token, $confirm, $productNo);
            dump($response);
            $this->assertCount(4, $response);
            $this->assertEquals(0, $response['code']);
        }
    ...
    protected function auditProduct($token, $confirm, $productNo)
    {
        $response = $this->json(
            'POST', '/api/admin/product/audit', array(
            "accessToken"=>$token,
            "confirm"=>$confirm,
            "productNo"=>$productNo,
            )
