...
class ActivitystepsTest extends TestCase
{
    /**
    ...
    public function testAddProductInfo()
    {
        ...
        dump('得到当前活动是否达到活动要求');
        $activityNo = 1400;
        $response = $this->activitysteps($token, $activityNo);
        dump($response);
        $this->assertCount(4, $response);
        $this->assertEquals(0, $response['code']);
    }

    protected function activitysteps($token, $activityNo)
    {
        $response = $this->json(
            'POST', '/api/login/user/activitysteps', array(
            "accessToken"=>$token,
