...
class GetCloseOrdersTest extends TestCase
{
    /**
    ...
    public function testAddProductInfo()
    {
        ...
        $pageIndex = '1';
        $coinType = 'KKC-BJ0001';
        $response = $this->getCloseOrders($token, $pageSize, $pageIndex, $coinType);
        dump($response);
        $this->assertCount(4, $response);
        $this->assertEquals(0, $response['code']);
    }

    protected function getCloseOrders($token, $pageSize, $pageIndex, $coinType)
    {
        $response = $this->json(
            'POST', '/api/login/trade/getcloseorders', array(
            "accessToken"=>$token,
            "pageSize"=>$pageSize,
            "pageIndex"=>$pageIndex,
            "coinType"=>$coinType,
