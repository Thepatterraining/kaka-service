...
class GetUserVoucherStoragesTest extends TestCase
{
    /**
    ...
    public function testGetTrendList()
    {
        ...
        $response = $this->getList($token, $pageIndex, $pageSize, $filters);
        dump($response);
        $this->assertCount(4, $response);
        $this->assertEquals(0, $response['code']);
    }

    protected function getList($token, $pageIndex, $pageSize, $filters)
    {
        $response = $this->json(
            'POST', '/api/user/getstorage', array(
            "accessToken"=>$token,
            "pageIndex"=>$pageIndex,
            "pageSize"=>$pageSize,
            "filters"=>$filters,
